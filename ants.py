    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                cond_1 = self.ant.can_contain(insect)
                cond_2 = insect.can_contain(self.ant)
                cond_3 = cond_1 or cond_2
                assert cond_3, 'Two ants in {0}'.format(self)
                if cond_1:
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        cond_1 = self.container
        try:
            cond_2 = self.ant is None
        except:
            cond_2 = False
        cond_3 = not other.container
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while place is not hive and found_bee is None and distance <= self.max_range:
            if self.min_range <= distance <= self.max_range:
                found_bee = random_or_none(place.bees)
...
class WallAnt(Ant):
    '''A WallAnt that does nothing each turn and has 4 armor value.'''
    name = 'Wall'
    food_cost = 4
    implemented = True 
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        for bee in bees_copy:
                ...
                if bee.armor <= 0:
                    Place.remove_insect(bee.place, bee)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        while place is not None:
            ...
            if cur_ant is not None:
                if isinstance(cur_ant, BodyguardAnt):
                    ants = [cur_ant, cur_ant.ant]
                    for ant in ants:
                        if ant is not None and ant not in QueenAnt.double_ants:
                            ant.damage *= 2
                            QueenAnt.double_ants.append(ant)
                else:
                    if cur_ant not in QueenAnt.double_ants:
                         cur_ant.damage *= 2
                         QueenAnt.double_ants.append(cur_ant)               
            place = place.exit
    ...
    def reduce_armor(self, amount):
        ...
        """
        if self.place.bees is not None and self.true_queen:
            bees_win()
        elif not self.true_queen:
            self.armor -= amount
            self.place.remove_insect(self)
...
def apply_effect(effect, bee, duration):
    ...
    original = bee.action
    applied = effect(original)

    def apply_function(colony):
        nonlocal duration
        if duration > 0:
            applied(colony)
        else:
